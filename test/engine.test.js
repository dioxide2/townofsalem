...
const ALLOWED = ['Villager', 'Jailor', 'Investigator', 'Doctor', 'Vigilante', 'Godfather', 'Mafioso', 'Jester'];
...
for (let n = 7; n <= 15; n++) {
  ...
  assert(jesters.length === (n >= 9 ? 1 : 0), `n=${n}: jester present iff 9+ (got ${jesters.length})`);
...
assert(mafiaCountFor(15) === 4, '15 players -> 4 mafia (headline requirement)');
  ...
  while (g.phase !== 'gameOver' && guard < 400) {
    ...
    switch (g.phase) {
      ...
      case 'voting':
        ...
        if (g.phase === 'voting') g.advancePhase();
        break;
